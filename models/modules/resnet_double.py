...
class ResNet(nn.Module):
    ...
    #    self.conv2 = nn.Conv2d(64, 64, kernel_size=3, stride=2, padding=1,
     ...
     #   self.bn2 = nn.BatchNorm2d(64)
        ...
        self.inplanes = 64
        self.layer11 = self._make_layer(block, 64, layers[0], stride=1)
        ...
        self.fc1_0 = nn.Linear(512*block.expansion, 100)
        self.fc1_1 = nn.Linear(100, 2)
        self.fc2_0 = nn.Linear(512*block.expansion, 100)
     ...
     #   x = self.conv2(x)
      #  x = self.bn2(x)
       # x = self.relu(x)
        #x = self.maxpool(x)
        xfen=x
        x12 = self.layer1(xfen)
        x12 = self.layer2(x12)
        x12 = self.layer3(x12)
        x12 = self.layer4(x12)
       # print(x.shape)
        x12 = self.avgpool(x12)        
        x12 = x12.view(x12.size(0), -1)
        x12 = self.fc1_0(x12)
        x12 = self.fc1_1(x12)
        
        x3 = self.layer11(xfen)
        x3 = self.layer21(x3)
        x3 = self.layer31(x3)
        x3 = self.layer41(x3)
       # print(x.shape)
